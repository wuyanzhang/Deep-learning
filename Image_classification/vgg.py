...
# 加载预训练模型
data_dict = np.load('E:/Project/tensorflow_model/AlexNet_classification/vgg16.npy', encoding='latin1').item()
...
def inference(images,_dropout, n_cls, batch_size):
    ...
    flatten = tf.reshape(pool5, [batch_size, -1])
    fc6 = fc(flatten, 4096, 'fc6', xavier=True)
    dropout1 = tf.nn.dropout(fc6, _dropout)
    fc6 = fc(dropout1, 4096, 'fc6', xavier=True)
    dropout1 = tf.nn.dropout(fc6, _dropout)
